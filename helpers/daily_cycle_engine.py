...
def should_run_morning():
    """Return True if current UTC time is in the morning run window (09:00-09:30)."""
    now = datetime.now(timezone.utc)
    return now.hour == 9 and now.minute < 30


def should_run_midday():
    """Return True if current UTC time is in the midday run window (13:00-13:30)."""
    now = datetime.now(timezone.utc)
    return now.hour == 13 and now.minute < 30


def should_run_close():
    """Return True if current UTC time is in the close run window (20:00-20:30)."""
    now = datetime.now(timezone.utc)
...
def run_cycle():
    ...
    if should_run_morning() and not state.get("morning_run"):
        state["morning_run"] = True
        save_cycle_state(state)
        return {"status": "ok", "message": "Morning cycle executed.", "timestamp": timestamp}

    if should_run_midday() and not state.get("midday_run"):
        state["midday_run"] = True
        save_cycle_state(state)
        return {"status": "ok", "message": "Midday cycle executed.", "timestamp": timestamp}

    if should_run_close() and not state.get("close_run"):
        state["close_run"] = True
        save_cycle_state(state)
