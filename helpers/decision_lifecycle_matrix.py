...
def compute_cross_horizon_alignment(snapshot_df):
    ...
    if avg_positive > 0.7:
        ...
        return "Weak", f"Weak alignment â majority of horizons show negative alpha ({avg_positive:.0%} positive)"
...
def compute_stage_4(decisions, scope, selected_wave):
    ...
    if counts["Awaiting Approval"] > 2:
        posture = "Elevated governance attention required â multiple decisions awaiting approval."
    elif counts["Awaiting Approval"] > 0:
        posture = "Normal governance posture â decisions are progressing through approval."
    else:
        posture = "Clear governance posture â no decisions pending approval."
...
def compute_stage_5(decisions, snapshot_df, attrib_df, scope, selected_wave):
    ...
    return _make_stage_response(
        ...
        bullets=[f"{item['id']}: {item['type']} â {item['monitoring_window']}" for item in items],
...
def compute_stage_7(decisions, attrib_df, scope, selected_wave):
    ...
    if len(with_outcomes) >= 2:
        ...
        if positive_outcomes:
            ...
            learnings.append(f"Positive outcomes observed in {', '.join(types)} decisions â these decision types may have stronger process alignment.")

        if negative_outcomes:
            types = set(d.get("decision_type", "Other") for d in negative_outcomes)
            learnings.append(f"Mixed or negative outcomes in {', '.join(types)} â consider whether timing or regime conditions played a role.")

        regime_decisions = [d for d in with_outcomes if d.get("regime_at_decision") == "Elevated Volatility"]
        if regime_decisions:
            learnings.append(f"{len(regime_decisions)} decision(s) made during elevated volatility â outcomes may be regime-influenced rather than process-driven.")
