...
def compute_alpha_attribution(
    ...
    """
    Compute comprehensive alpha attribution decomposition for a Wave.
    
    This function integrates with the alpha_attribution module to provide
    precise, reconciled decomposition of Wave alpha into 5 components:
    ...
    Returns:
        Tuple of (daily_attribution_df, summary_dict)
        - daily_attribution_df: DataFrame with daily-level attribution
    ...
    try:
        ...
        raise ImportError(
            "alpha_attribution module not found. "
            "Ensure alpha_attribution.py is in the same directory as waves_engine.py"
        )
    
    # Get history with diagnostics
    history_df = compute_history_nav(wave_name, mode=mode, days=days, include_diagnostics=True)
    
    if history_df is None or history_df.empty:
        return pd.DataFrame(), {
            "ok": False,
            "message": "No history data available for attribution"
        }
    
    # Extract diagnostics
    diagnostics_df = history_df.attrs.get("diagnostics")
    
    # Get parameter defaults for this wave/mode
    params = get_parameter_defaults(wave_name, mode)
    tilt_strength = params.get("tilt_strength", DEFAULT_TILT_STRENGTH)
    base_exposure = params.get("base_exposure", MODE_BASE_EXPOSURE.get(mode, 1.0))
