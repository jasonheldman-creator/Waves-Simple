  ...
  return (
    <section className="bg-gradient-to-b from-gray-900 to-black py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div
          className={`grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12 items-center ${reverse ? "lg:flex-row-reverse" : ""}`}
        >
          {/* Screenshot */}
          <div className={`${reverse ? "lg:order-2" : ""}`}>
            <div className="relative rounded-lg border border-cyan-500/20 bg-gray-900/50 p-2 shadow-lg shadow-cyan-500/10">
              <div className="relative aspect-video w-full bg-gray-800 rounded overflow-hidden">
                <Image src={imagePath} alt={screenshot.alt} fill className="object-contain" />
              </div>
            </div>
          </div>
          ...
          <div className={`${reverse ? "lg:order-1" : ""}`}>
            <h3 className="text-2xl font-bold text-white sm:text-3xl mb-6">{screenshot.title}</h3>
