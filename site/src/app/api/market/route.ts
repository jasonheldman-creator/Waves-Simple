...
import { DEFAULT_MARKET_DATA, type MarketData, type MarketSymbol, type DataState } from "@/types/market";
...
function determineMarketRegime(symbols: MarketSymbol[]): MarketData['regime'] {
  ...
  let description = 'Market conditions showing balanced risk/reward dynamics with moderate volatility.';
  let exposureImplications = 'Maintain balanced exposure across growth and defensive sectors. Monitor regime transitions.';
  ...
  if (spy) {
    ...
    if (vix && vix.value > 25) {
      regime = 'Risk-Off';
      description = 'Elevated volatility with defensive market behavior. Heightened uncertainty driving risk aversion.';
      exposureImplications = 'Reduce growth exposure. Increase defensive positions and consider hedging strategies.';
    } else if (dailyChange > 1.0) {
      regime = 'Risk-On';
      description = 'Strong market momentum with risk asset outperformance. Bullish sentiment prevailing.';
      exposureImplications = 'Favor growth and cyclical sectors. Monitor for overextension and prepare for rotation.';
    } else if (dailyChange < -1.0) {
      regime = 'Risk-Off';
      description = 'Market showing defensive characteristics. Risk assets underperforming safe havens.';
      exposureImplications = 'Reduce growth exposure. Increase defensive positions and quality bias.';
    }
  }
