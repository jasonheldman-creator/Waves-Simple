...
if yf is not None:
    try:
        ...
        if not vix_df.empty:
            # vix_df may have "^VIX" or "Adj Close" depending on yf behavior; handle common cases
...
with tabs[12]:
    ...
    for n in notes:
        st.markdown(f"- {n}")
    render_definitions(keys=["Decision Intelligence"], title="Definitions (IC Notes)")
