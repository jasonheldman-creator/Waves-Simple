...
# Discover available waves from performance logs
...
wave_label_map = {w: w for w in waves}
selected_wave_file = st.sidebar.selectbox("Select Wave", waves, format_func=lambda w: wave_label_map[w])

wave_name = selected_wave_file  # same string used in filenames
...
with col1:
    st.metric(
        "Latest NAV",
        f"{perf_df['nav'].iloc[-1]:.4f}",
    )
    st.metric(
        "Latest Alpha (since last point)",
        f"{perf_df['alpha'].iloc[-1]*100:.2f}%",
    )

with col2:
    st.line_chart(
        perf_df.set_index("timestamp")[["nav", "benchmark_price"]]
    )

st.caption("NAV vs Benchmark Price (not scaled)")

# -------------------------------------------------------------------
# Latest positions
# -------------------------------------------------------------------
# Grab the most recent positions file for this wave
pos_files = sorted(POSITIONS_DIR.glob(f"{wave_name}_positions_*.csv"))
if not pos_files:
    st.warning(f"No positions logs found for {wave_name}.")
else:
    latest_pos_path = pos_files[-1]
    pos_df = pd.read_csv(latest_pos_path)

    st.subheader(f"Latest Positions — {wave_name}")
    st.dataframe(pos_df)
