...
#       - Rolling Alpha / Rolling Vol charts
...
#   ✅ st.set_page_config is the FIRST Streamlit command (prevents blank-screen crashes)
#   ✅ Removed debug st.stop()
#   ✅ Engine import + get_all_waves/get_modes wrapped with clear error UI
#   ✅ Optional plotly/yfinance handled safely (no hard crash)
...
# ============================================================
# Engine import (guarded — show error instead of blank screen)
# ============================================================
ENGINE_OK = True
ENGINE_ERR = None
try:
...
def compute_wave_history(wave_name: str, mode: str, days: int = 365) -> pd.DataFrame:
    try:
        if we is None:
            raise RuntimeError("waves_engine not available")
...
def get_benchmark_mix() -> pd.DataFrame:
    try:
        if we is None:
            raise RuntimeError("waves_engine not available")
        out = we.get_benchmark_mix_table()
        if out is None:
            out = pd.DataFrame()
        return out
    except Exception:
...
def get_wave_holdings(wave_name: str) -> pd.DataFrame:
    try:
        if we is None:
            raise RuntimeError("waves_engine not available")
        out = we.get_wave_holdings(wave_name)
        if out is None:
            out = pd.DataFrame()
        return out
    except Exception:
...
# ============================================================
# Sidebar: waves & modes (guarded)
# ============================================================
if not ENGINE_OK:
    st.error("❌ waves_engine failed to import (this causes a blank screen if not handled).")
...
try:
    ...
    st.error("❌ Engine call failed: get_all_waves()")
    st.exception(e)
    st.stop()
...
with st.sidebar:
    ...
    show_boot = st.toggle("Show boot diagnostics", value=False)
...
if show_boot:
    with st.sidebar:
        st.markdown("---")
        st.markdown("**Boot diagnostics**")
        st.write("engine_ok:", ENGINE_OK)
        st.write("yfinance:", bool(yf is not None))
        st.write("plotly:", bool(go is not None))
        st.write("waves:", len(all_waves))
        st.write("modes:", len(all_modes))
...
# ============================================================
# Pinned Summary Bar (Sticky) + Governance chips
# ============================================================
...
if not h_bar.empty and len(h_bar) >= 2 and all(c in h_bar.columns for c in ["wave_nav", "bm_nav", "wave_ret", "bm_ret"]):
