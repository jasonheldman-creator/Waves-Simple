...
# Import decision ledger and alpha contracts modules
try:
...
try:
    ...
    ALPHA_CONTRACTS_AVAILABLE = True
except ImportError:
    ALPHA_CONTRACTS_AVAILABLE = False
...
def render_decision_ledger_section():
    """
    Render the Decision Ledger + Alpha Contracts governance section.
    Allows users to compute metrics, log decisions, and evaluate contracts.
    """
    ...
    try:
        ...
        with col1:
            selected_wave = st.selectbox(
                "Select Wave",
                options=waves,
                key="decision_ledger_wave_selector",
                help="Choose a wave to compute and log decision snapshot"
            )
        ...
        with col2:
            period_days = st.selectbox(
                ...
                key="decision_ledger_period",
                help="Select the lookback period for metrics"
            )
        ...
        # Compute & Log button
        if st.button(
            "🎯 Compute & Log Decision Snapshot",
            key="compute_log_decision",
            use_container_width=True,
            type="primary"
        ):
            with st.spinner(f"Computing metrics for {selected_wave}..."):
                try:
                    ...
                    if wave_data is None or len(wave_data) == 0:
                        ...
                        # Add additional metrics for ledger
                        if 'vix' in wave_data.columns:
                            metrics['vix'] = wave_data['vix'].iloc[-1] if len(wave_data) > 0 else None
                        
                        if 'exposure' in wave_data.columns:
                            metrics['exposure'] = wave_data['exposure'].iloc[-1] if len(wave_data) > 0 else None
                        
                        # Validate and collect warnings
                        warnings = validate_ledger_entry(metrics)
                        
                        # Append to ledger
                        success = append_decision_snapshot(
                            wave_name=selected_wave,
                            mode="Standard",  # Could be configurable
                            period=period_days,
                            metrics=metrics,
                            warnings=warnings
                        )
                        ...
                        if success:
                            st.success(f"✅ Decision snapshot logged for {selected_wave}")
                            if warnings:
                                st.warning(f"⚠️ Warnings: {'; '.join(warnings)}")
                        else:
                            st.error("❌ Failed to log decision snapshot")
        ...
        try:
            ...
            if wave_data is not None and len(wave_data) > 0:
                ...
                # Evaluate contract
                status, reasons = evaluate_contract(selected_wave, metrics)
                
                # Display status with color coding
                status_color = get_contract_status_color(status)
                ...
                # Show contract details in expander
                contract = get_contract(selected_wave)
                if contract:
                    with st.expander("📄 View Contract Details"):
                        st.markdown(format_contract_summary(contract))
            else:
                st.info(f"ℹ️ No data available for {selected_wave} contract evaluation")
        
        except Exception as e:
            st.warning(f"⚠️ Contract evaluation unavailable: {str(e)}")
        ...
        try:
            # Read ledger filtered by selected wave
            ledger_df = read_ledger(wave_name=selected_wave, limit=25)
            ...
            if ledger_df is not None and len(ledger_df) > 0:
                ...
                numeric_cols = ['vix', 'exposure', 'selection_alpha', 'overlay_alpha', 
                               'risk_off_alpha', 'total_alpha', 'cumulative_return', 
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"decision_ledger_{selected_wave}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv"
                )
            else:
                st.info(f"ℹ️ No ledger entries found for {selected_wave}")
            
            # Show ledger stats
            stats = get_ledger_stats()
            if stats['total_entries'] > 0:
                st.caption(f"Total entries: {stats['total_entries']} | Waves tracked: {stats['waves_tracked']}")
        
        except Exception as e:
            st.warning(f"⚠️ Unable to load ledger: {str(e)}")
    
    except Exception as e:
        st.error(f"❌ Error in Decision Ledger section: {str(e)}")
...
def render_sidebar_info():
    ...
    # Display reload token if it exists
    if "reload_token" in st.session_state and st.session_state.reload_token > 0:
        st.sidebar.caption(f"🔄 Reload token: {st.session_state.reload_token}")
...
def render_executive_tab():
    ...
    # Decision Ledger + Alpha Contracts Section - NEW
    render_decision_ledger_section()
...
def render_compare_tab():
    """
    Render the Compare tab for comprehensive side-by-side Wave comparisons.
    
    This tab provides CIO decision-making intelligence with:
    - Multi-select control for Waves (2-6 waves, defaulting to top 4 by WaveScore)
    - Period selector (15D, 30D, 60D, YTD)
    - Dynamic comparison table with key metrics
    - Conditional formatting highlighting best values
    - "Why this matters" caption
    - Raw input data expander for auditability
    - CSV download capability
    """
    st.header("⚖️ Compare - Side-by-Side Wave Comparison")
    st.write("Comprehensive comparison matrix for CIO decision-making processes.")
    ...
    if not waves or len(waves) == 0:
        st.error("❌ No waves available for comparison. Please check data sources.")
        return
    
    # Calculate WaveScores for all waves to determine defaults
    wave_scores = []
    for wave_name in waves:
        try:
            # Use centralized wave history loader
            ...
            if wave_data is not None and len(wave_data) > 0:
                ...
                wave_scores.append((wave_name, 0))
        except Exception:
            wave_scores.append((wave_name, 0))
    
    # Sort by WaveScore descending and get top 4
    wave_scores.sort(key=lambda x: x[1], reverse=True)
    top_4_waves = [w[0] for w in wave_scores[:4]]
    
    # Ensure we have at least 2 waves
    if len(top_4_waves) < 2:
        default_waves = waves[:min(4, len(waves))]
    else:
        default_waves = top_4_waves
    ...
    with col2:
        period_options = {
            ...
            "30D": 30,
            "60D": 60,
            "YTD": 252  # Approximate trading days in a year
        }
    ...
    # Validate wave selection
    if len(selected_waves) < 2:
        st.warning("⚠️ Please select at least 2 waves for comparison.")
        return
    
    if len(selected_waves) > 6:
        st.warning("⚠️ Please select no more than 6 waves for comparison.")
        return
    
    # ========================================================================
    # SECTION 2: BUILD COMPARISON TABLE
    # ========================================================================
        ...
        try:
            ...
            if wave_data is None or len(wave_data) == 0:
                ...
                row = {
                    'Wave': wave_name,
                    'Return': 'Unavailable',
                    'Alpha vs Benchmark': 'Unavailable',
                    'Volatility': 'Unavailable',
                    'Max Drawdown': 'Unavailable',
                    'Beta': 'Unavailable',
                    'Benchmark Drift': 'Unavailable',
                    'Avg Exposure': 'Unavailable',
                    'Avg Safe Fraction': 'Unavailable',
                    'Alpha Truth Status': 'Unavailable',
            ...
            raw_data_dict[wave_name] = wave_data.copy()
            
            # Calculate metrics
            metrics = calculate_wave_metrics(wave_data)
            
            # Return
            cumulative_return = metrics.get('cumulative_return', 'N/A')
            return_str = f"{cumulative_return * 100:.2f}%" if isinstance(cumulative_return, (int, float)) else 'Unavailable'
            
            # Alpha vs Benchmark
            cumulative_alpha = metrics.get('cumulative_alpha', 'N/A')
            alpha_str = f"{cumulative_alpha * 100:.2f}%" if isinstance(cumulative_alpha, (int, float)) else 'Unavailable'
            
            # Volatility
            volatility = metrics.get('volatility', 'N/A')
            volatility_str = f"{volatility * 100:.2f}%" if isinstance(volatility, (int, float)) else 'Unavailable'
            
            # Max Drawdown
            max_drawdown = metrics.get('max_drawdown', 'N/A')
            drawdown_str = f"{max_drawdown * 100:.2f}%" if isinstance(max_drawdown, (int, float)) else 'Unavailable'
            
            # Beta (or beta drift)
            beta = 'Unavailable'
            if 'portfolio_return' in wave_data.columns and 'benchmark_return' in wave_data.columns:
                try:
                    # Calculate beta as covariance(portfolio, benchmark) / variance(benchmark)
                    cov_matrix = wave_data[['portfolio_return', 'benchmark_return']].cov()
                    if len(cov_matrix) > 1:
                        covariance = cov_matrix.iloc[0, 1]
                        variance = wave_data['benchmark_return'].var()
                        if variance > 0:
            ...
            # Benchmark Drift
            benchmark_drift = 'Unavailable'
            if 'benchmark_return' in wave_data.columns:
                try:
                    benchmark_cumulative = wave_data['benchmark_return'].sum()
                    benchmark_drift = f"{benchmark_cumulative * 100:.2f}%"
                except Exception:
                    benchmark_drift = 'Unavailable'
            
            # Avg Exposure
            avg_exposure = 'Unavailable'
            if 'exposure' in wave_data.columns:
                try:
                    avg_exp = wave_data['exposure'].mean()
                    avg_exposure = f"{avg_exp * 100:.1f}%"
                except Exception:
                    avg_exposure = 'Unavailable'
            
            # Avg Safe Fraction
            avg_safe = 'Unavailable'
            if 'safe_pct' in wave_data.columns:
                try:
                    avg_safe_val = wave_data['safe_pct'].mean()
                    avg_safe = f"{avg_safe_val * 100:.1f}%"
                except Exception:
                    avg_safe = 'Unavailable'
            elif 'cash_pct' in wave_data.columns:
                try:
                    avg_safe_val = wave_data['cash_pct'].mean()
                    avg_safe = f"{avg_safe_val * 100:.1f}%"
                except Exception:
                    avg_safe = 'Unavailable'
            
            # Alpha Truth Status/Score
            alpha_truth_status = 'Unavailable'
            try:
                trust_signal = compute_trust_signal(wave_name=wave_name, period_days=period_days)
                if trust_signal:
                    alpha_truth_status = f"{trust_signal.status} ({trust_signal.score})"
            except Exception:
                alpha_truth_status = 'Unavailable'
            ...
            if ALPHA_CONTRACTS_AVAILABLE:
                try:
                    ...
                    if contract:
                        evaluation = evaluate_contract(wave_data, contract)
                        if evaluation and hasattr(evaluation, 'status'):
                            contract_status = evaluation.status.value
                except Exception:
                    contract_status = 'Unavailable'
            ...
            row = {
                ...
                'Beta': beta,
                'Benchmark Drift': benchmark_drift,
                'Avg Exposure': avg_exposure,
                'Avg Safe Fraction': avg_safe,
                'Alpha Truth Status': alpha_truth_status,
            ...
            # Graceful error handling - don't crash, just mark as unavailable
            row = {
                'Wave': wave_name,
                'Return': 'Unavailable',
                'Alpha vs Benchmark': 'Unavailable',
                ...
                'Alpha Truth Status': 'Unavailable',
                'Contract Status': 'Unavailable',
                'Data Age': f'Error: {str(e)}'
            }
            comparison_data.append(row)
            raw_data_dict[wave_name] = f"Error loading data: {str(e)}"
    ...
    # ========================================================================
    # SECTION 3: DISPLAY TABLE WITH CONDITIONAL FORMATTING
    # ========================================================================
    
    # Apply conditional formatting using Streamlit's dataframe styling
    # We'll use a simple approach: highlight best values in each column
    
    def highlight_best(s, higher_is_better=True):
        """
        Highlight the best value in a series.
        For numeric columns, extract numeric values and highlight best.
        """
        try:
            # Extract numeric values from strings
            numeric_values = []
            for val in s:
                if val == 'Unavailable' or val == 'N/A':
                    ...
                    try:
                        ...
                        if 'GREEN' in clean_val or 'YELLOW' in clean_val or 'RED' in clean_val:
                            # Extract score if present
                            if '(' in clean_val and ')' in clean_val:
                                score_str = clean_val.split('(')[1].split(')')[0]
                                numeric_values.append(float(score_str))
                            else:
                                numeric_values.append(None)
                        elif 'days' in clean_val:
                            # Extract days
                            days_str = clean_val.split()[0]
                            numeric_values.append(float(days_str))
                        else:
            ...
            # Create highlighting
            styles = []
            for i, (val, num_val) in enumerate(zip(s, numeric_values)):
                if num_val is not None and abs(num_val - best_val) < 0.001:
    ...
    # ========================================================================
    # SECTION 4: WHY THIS MATTERS CAPTION
    # ========================================================================
    
    st.caption("""
    **Why This Matters:** This comparison matrix provides CIOs and investment committees with a comprehensive 
    side-by-side view of Wave performance across key decision-making metrics. By evaluating Return, Alpha generation, 
    Risk characteristics (Volatility, Max Drawdown), Market exposure (Beta, Exposure levels), and Trust signals 
    (Alpha Truth status, Contract compliance), decision-makers can identify the optimal Wave allocations for their 
    portfolio objectives. The multi-period view enables both tactical (15D, 30D) and strategic (60D, YTD) assessment 
    of Wave efficacy and risk-adjusted performance.
    """)
    
    st.divider()
    
    # ========================================================================
    # SECTION 5: RAW INPUT DATA EXPANDER
    # ========================================================================
    
    with st.expander("📋 Raw Input Data (Auditability)", expanded=False):
        st.write("**Raw wave data used for comparison calculations:**")
        
        for wave_name, wave_data in raw_data_dict.items():
            ...
            if isinstance(wave_data, str):
                ...
                # Show data summary
                st.caption(f"Rows: {len(wave_data)}, Columns: {len(wave_data.columns)}")
            
            st.markdown("---")
    
    # ========================================================================
    # SECTION 6: CSV DOWNLOAD BUTTON
    # ========================================================================
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"Wave_Comparison_{selected_period}_{timestamp}.csv"
        ...
        st.success(f"✅ Comparison table ready for download ({len(selected_waves)} waves, {selected_period} period)")
        
    except Exception as e:
        st.error(f"❌ Error generating CSV download: {str(e)}")
    
    # Footer
    st.markdown("---")
    st.caption(f"Compare Tab - WAVES Intelligence™ | Period: {selected_period} | Waves: {len(selected_waves)}")
